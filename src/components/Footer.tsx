"use client";

import { useEffect, useState } from "react";
import Image from "next/image";
import Link from "next/link";
import { Phone, Lock } from "lucide-react";

interface FooterProps {
  selectedSchoolCode?: string;
}

export default function Footer({ selectedSchoolCode = "sadjati" }: FooterProps) {
  const [siteProfile, setSiteProfile] = useState<any>(null);

  useEffect(() => {
    fetch(`/api/site-profile?schoolCode=${selectedSchoolCode}`)
      .then((r) => {
        const contentType = r.headers.get("content-type");
        if (r.ok && contentType && contentType.includes("application/json")) {
          return r.json();
        }
        return { success: false, data: null };
      })
      .then((data) => {
        if (data.success && data.data) {
          setSiteProfile(data.data);
        }
      })
      .catch(() => {});
  }, [selectedSchoolCode]);

  const schoolName = siteProfile?.school?.name || "Smart Kids";
  const phone = siteProfile?.phone || "0812 3456 7890";
  const instagram = siteProfile?.instagram || "smartkids";
  const facebook = siteProfile?.facebook || "Smart Kids";
  const tagline =
    siteProfile?.schoolSubtitle ||
    `${schoolName} adalah lembaga pendidikan anak di bawah YAPCHI Foundation yang menghadirkan pembelajaran menyenangkan.`;

  return (
    <footer id="footer" className="bg-linear-to-r from-blue-900 via-blue-800 to-indigo-900 text-white pt-10 pb-8 mt-auto border-t border-blue-700">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center pb-8 border-b border-blue-400/30">
          {/* Brand Info (Left) */}
          <div className="md:col-span-6 flex flex-col sm:flex-row items-start gap-4">
            <div className="relative w-20 h-20 shrink-0 flex items-center justify-center">
              <Image
                src={siteProfile?.school?.logoUrl || "/images/smart_kids_logo.png"}
                alt="Smart Kids Logo"
                width={80}
                height={80}
                className="object-contain"
              />
            </div>
            <div>
              <h3 className="font-extrabold text-xl text-white tracking-wide">
                {schoolName}
              </h3>
              <p className="text-blue-100 text-sm mt-1 leading-relaxed">
                {tagline}
              </p>
            </div>
          </div>

          {/* Contact Details (Middle Right) */}
          <div className="md:col-span-4 flex flex-col space-y-2.5">
            <h4 className="font-bold text-base text-white tracking-wide mb-1">
              Hubungi Kontak Cabang
            </h4>
            {(() => {
              const cleanPhone = phone ? phone.replace(/[^0-9]/g, "") : "";
              const waPhone = cleanPhone.startsWith("0") ? "62" + cleanPhone.slice(1) : cleanPhone;
              const igUser = (instagram || "").replace(/^@/, "").trim();
              const fbTarget = (facebook || "").trim();

              return (
                <>
                  <a
                    href={waPhone ? `https://wa.me/${waPhone}` : `tel:${phone}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-3 text-sm text-blue-100 hover:text-amber-300 transition-colors group cursor-pointer"
                    title={`Hubungi via WhatsApp (${phone})`}
                  >
                    <div className="w-7 h-7 rounded-full bg-blue-500/40 group-hover:bg-blue-500/70 flex items-center justify-center shrink-0 transition-colors">
                      <Phone className="w-3.5 h-3.5" />
                    </div>
                    <span className="group-hover:underline">{phone}</span>
                  </a>

                  <a
                    href={igUser ? `https://instagram.com/${igUser}` : "https://instagram.com"}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-3 text-sm text-blue-100 hover:text-amber-300 transition-colors group cursor-pointer"
                    title={`Buka Instagram @${igUser}`}
                  >
                    <div className="w-7 h-7 rounded-full bg-blue-500/40 group-hover:bg-blue-500/70 flex items-center justify-center shrink-0 transition-colors">
                      <svg className="w-3.5 h-3.5 fill-current" viewBox="0 0 24 24">
                        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                      </svg>
                    </div>
                    <span className="group-hover:underline">{instagram}</span>
                  </a>

                  <a
                    href={fbTarget ? (fbTarget.startsWith("http") ? fbTarget : `https://facebook.com/${encodeURIComponent(fbTarget)}`) : "https://facebook.com"}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-3 text-sm text-blue-100 hover:text-amber-300 transition-colors group cursor-pointer"
                    title={`Buka Facebook ${facebook}`}
                  >
                    <div className="w-7 h-7 rounded-full bg-blue-500/40 group-hover:bg-blue-500/70 flex items-center justify-center shrink-0 transition-colors">
                      <svg className="w-3.5 h-3.5 fill-current" viewBox="0 0 24 24">
                        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                      </svg>
                    </div>
                    <span className="group-hover:underline">{facebook}</span>
                  </a>
                </>
              );
            })()}
          </div>

          {/* YAPCHI Emblem (Far Right) */}
          <div className="md:col-span-2 flex justify-start md:justify-end items-center">
            <div className="relative w-24 h-24 shrink-0 transform hover:scale-105 transition-transform flex items-center justify-center">
              <Image
                src="/images/yapchi_logo.png"
                alt="YAPCHI Emblem"
                width={96}
                height={96}
                className="object-contain"
              />
            </div>
          </div>
        </div>

        {/* Bottom copyright notice & Portal Akun Link */}
        <div className="pt-6 text-center text-xs text-blue-200 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p>© {new Date().getFullYear()} {schoolName} by Elevore. Hak Cipta Dilindungi.</p>
          <Link
            href="/admin/login"
            className="inline-flex items-center gap-1.5 px-3 py-1 bg-blue-600/40 hover:bg-blue-600/70 text-blue-100 rounded-full transition-colors text-[11px]"
          >
            <Lock className="w-3 h-3" />
            <span>Portal Akun</span>
          </Link>
        </div>
      </div>
    </footer>
  );
}
